...
### Requirement: 数学题目生成
(SHALL) 系统应能生成五种数学运算类型的题目：加法、减法、乘法、有余数除法、无余数除法。生成的题目应符合配置的数值范围限制。系统应根据选择的难度级别 (LV1/LV2/LV3/LV4/LV5) 调整题目参数,包括数值范围和可用的运算类型。为避免概率偏差，系统应在练习开始时预先分配每种题型的题目数量，并在出题过程中从分配的题型池中随机抽取，确保各题型在有限题目数内尽可能均匀分布。系统应使用单一的 Random 实例来生成所有随机数，避免在每次生成题目时创建新的 Random 实例，以确保随机数的真正随机性和避免重复生成相同的题目序列。除法策略应正确使用 intBits 参数，确保除数和商在正确的范围内独立生成，避免除数和商总是相同的情况。难度定义如下:
...
#### Scenario: 无余数除法使用正确的 intBits 计算
- **GIVEN** intBits = 1（个位数除法）
- **WHEN** 生成无余数除法题目
- **THEN** 除数和商的范围应为 1-9
- **AND** 使用公式 `max = Math.Pow(10, intBits)`，即 max = 10
- **AND** 除数和商独立生成，避免总是相同
- **AND** 题目例子：7 ÷ 3 = 2, 9 ÷ 2 = 4（而非 8 ÷ 8 = 1）

#### Scenario: 无余数除法两位数计算
- **GIVEN** intBits = 2（两位数除法）
- **WHEN** 生成无余数除法题目
- **THEN** 除数和商的范围应为 10-99
- **AND** 使用公式 `max = Math.Pow(10, intBits)`，即 max = 100
- **AND** 除数和商独立生成

#### Scenario: 有余数除法使用相同的 intBits 计算
- **GIVEN** intBits = 1 或 intBits = 2
- **WHEN** 生成有余数除法题目
- **THEN** 使用与无余数除法相同的 intBits 计算公式
- **AND** 确保两种除法策略对 intBits 的解释一致
- **AND** intBits = 1 时，范围 1-9
- **AND** intBits = 2 时，范围 10-99
