...
![Version](https://img.shields.io/badge/version-v7.1.1-orange)

# 软件使用说明

## 简要介绍

BB Math 是一款面向儿童的小学数学练习软件，支持加法、减法、乘法、除法等多种题型，提供难度选择、金币奖励机制等功能，帮助孩子在趣味中提高数学能力。

## 主要功能

- ✅ **5种题型**：加法、减法、乘法、无余数除法、有余数除法
- ✅ **5个难度级别**：从个位数加减（LV1）到三位数四则运算（LV5）
- ✅ **金币奖励系统**：答对奖励金币，全对额外奖励
- ✅ **暂停机制**：支持限次数暂停或限时间暂停，防止过度使用
- ✅ **练习历史记录**：完整记录练习过程，方便查看学习进度
...
1. 访问项目发布页面：https://github.com/ngbruce/BB_Math/releases
2. 下载最新版本的压缩包（.zip 文件）
3. 解压文件到非系统目录（如 `D:\算术练习\`）
...
**注意**：
- 程序为绿色软件，无需安装，解压即用
- 应该在Windows系统给孩子建立一个普通账户，不要解压到C盘，否则孩子有可能运行时没有访问权限。
- 不需要管理员权限即可运行
...
- 文件命名格式：`bbmath_YYYYMMDD.log`（如 `bbmath_20260117.log`）
